  ...
  if (quizzes.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-border py-8 text-center">
        ...
        <p className="text-sm text-muted-foreground">{messages.learn.noQuizzesYet}</p>
      </div>
    );
  }
        ...
        <div key={quiz.id} className="flex items-center justify-between rounded-xl border border-border px-4 py-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {messages.learn.questionCount.replace("{count}", String(quiz.questions.length))}
            </p>
          </div>
          <Button size="sm" variant="outline" onClick={() => setActiveQuizId(quiz.id)}>
            {messages.learn.takeQuiz}
          </Button>
        </div>
      ))}
    </div>
